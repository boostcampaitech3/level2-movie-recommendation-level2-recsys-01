...
def main():
    ...
    csv_name = args.model + "_" + str(args.embedding_dim) + "_" + str(args.dropout) + "_" + args.activation + "_" + str(args.batch_size) + "_" + str(args.num_epochs)
    print(csv_name)
    ...
    fixlen_feature_columns = [SparseFeat(feat, data[feat].nunique(), embedding_dim=args.embedding_dim)
                                for feat in sparse_features]

    varlen_feature_columns = [VarLenSparseFeat(SparseFeat('genre', vocabulary_size=len(
        key2index) + 1, embedding_dim=args.embedding_dim), maxlen=max_len, combiner='mean')]  # Notice : value 0 is for padding for sequence input feature
    ...
    if args.model == "AutoInt":
        model = AutoInt(linear_feature_columns, dnn_feature_columns,
                        task='binary', l2_reg_embedding=1e-5, 
                        dnn_dropout=args.dropout, dnn_activation=args.activation,
                        device=device)
    elif args.model == "DCNMix":
    ...
    history = model.fit(model_input, data[target].values, batch_size=args.batch_size, epochs=args.num_epochs, verbose=2, validation_split=0.2)
    joblib.dump(model, "model/" + csv_name + '.pkl')
    ...
    submission.to_csv("output/" + csv_name + ".csv", index=False)
